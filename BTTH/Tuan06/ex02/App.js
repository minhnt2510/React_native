...
function HomeScreen({ navigation }) {
  ...
  const colorKeys = React.useMemo(() => {
    ...
    return product.colors.map((c) => (typeof c === 'string' ? c : c.key)).filter(Boolean);
  }, [product]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setProduct(data);
        const keys = (data.colors || []).map((c) => (typeof c === 'string' ? c : c.key)).filter(Boolean);
        setSelectedColor(keys.includes('red') ? 'red' : (keys[0] || 'blue'));
      } catch (e) {
      ...
      <View style={styles.card}>
        ...
        >
          <Text style={styles.optionBtnText}>
            {(colorKeys.length || product.colorsCount || 0)} MÀU - CHỌN MÀU
          </Text>
